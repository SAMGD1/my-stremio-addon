...
//   CSV_DIR             (optional) default "data"
//
// CSV columns (header names):
//   Const  (IMDb id, e.g. tt1234567)   |  Title  |  Release Date  |  Year
// A CSV filename containing "series" => that list is treated as series, else movie.
...
if (!ADMIN_PASSWORD) {
  console.warn("WARNING: ADMIN_PASSWORD is not set. Set it on Render to access /admin.");
}
...
async function ghGetFileSha(relpath) {
  try {
    const data = await ghRequest("GET", `/contents/${encodeURIComponent(relpath)}?ref=${encodeURIComponent(GITHUB_BRANCH)}`);
    return data.sha;
  } catch (e) {
    return null;
  }
...
async function ghPutCSV(filename, base64Content) {
  ...
  const sha = await ghGetFileSha(rel); // if exists, include sha to update
  const body = {
  ...
  for (const f of files) {
    ...
    const items = rows.map(r => {
      ...
      return {
        ...
        type: kind, // REAL type on item (important so streams load)
...
// enrich minimal meta for catalog cards (posters, rating/runtime for sort)
...
// meta handler: ALWAYS return real type so Stremio triggers stream add-ons
...
// mount the addon interface with optional secret check
const addonInterface = builder.getInterface();
app.use((req, res, next) => {
  // protect addon routes only (manifest/catalog/meta)
  if (/\/(manifest\.json|catalog|meta)/.test(req.url)) {
